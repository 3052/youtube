...
func get_watch(id string) (string, error) {
   ...
   req.Header["X-Goog-Visitor-Id"] = []string{id}
   req.Method = "POST"
   req.ProtoMajor = 1
   req.ProtoMinor = 1
   req.URL = &url.URL{}
   ...
   //req.Body = io.NopCloser(body_2)
   req.Body = io.NopCloser(bytes.NewReader(body_2.Marshal()))
   ...
   return "", errors.New("get_watch")
}

var body_2 = protobuf.Message{
   1: {protobuf.Unknown{
