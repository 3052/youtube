...
func visitor_id() string {
   to_string := base64.RawURLEncoding.EncodeToString
   encode := to_string(
      []byte{0xb7, 0xdf, 0xf1, 0x77, 0xe, 0x2, 0x5e, 0x0},
   )
   return to_string(protobuf.Message{1: {protobuf.Bytes(encode)}}.Marshal())
}
